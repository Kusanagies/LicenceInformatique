...
#define TAILLE_MATRICE	20 //On va prendre une matrice de taille 10 pour que ce soit plus "Simple"
							//On l'a testé avec la valeur 10 et 20 pour que ca "rentre" dans le terminale de commande
							//Avec la SDL on pourra faire un truc plus grand
...
int main(){
	...
	*/
	...
	printf("Création de fichier avec la matrice final du jeu de la vie appellé : FinalMatrix.txt");
	fp = fopen("FinalMatrix.txt","w");
	for(i = 0;i < TAILLE_MATRICE;i++){
		for(j = 0;j < TAILLE_MATRICE;j++){
			fprintf(fp,"%d",tabGameOfLife[i][j]);
		}
		fprintf(fp,"\n");
	}	
